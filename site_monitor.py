...
class SiteMonitor(Thread):
    ...
    def update_metrics(self, delay, duration):
        ...
        """
        _, codes_count, max_elapsed, avg_elapsed = self.get_metrics(self.last_updates[delay], duration, delay)
        self.metrics_sem.acquire()
        self.last_updates[delay] = time.time()
        self.metrics[delay] = {'codes_count': codes_count, 'max_elapsed': max_elapsed, 'avg_elapsed': avg_elapsed}
    ...
    def update_availability(self):
        ...
        """
        availability, _, _, _ = self.get_metrics(self.last_updates[120], 120, 120)
        self.metrics_sem.acquire()
